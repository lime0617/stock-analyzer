...
if analyze_button and user_input:
    with st.spinner(f"正在分析 {user_input}..."):
        try:
            ...
            # 价格 + 均线
            fig.add_trace(go.Scatter(x=df.index, y=df['Close'], name='收盘价', line=dict(width=3)), row=1, col=1)
            ...
            fig.update_layout(height=850, title_text="")
            ...
            st.subheader("💡 买卖建议")
            if final_score >= 8:
                st.success("✅ **建议考虑买入**（趋势较好）")
            elif final_score >= 6.5:
                st.info("🟡 **可以观察，回调时分批买入**")
            else:
